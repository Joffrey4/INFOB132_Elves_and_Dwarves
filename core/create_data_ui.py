...
def create_data_ui(data_map):
    ...
    for i in range(1, data_map['map_size'] + 1):
        data_ui[i + 4] = border_coloured_left + border_black + Fore.BLACK + ' ' + ('0' + str(i))[-2:] + ' ' + Style.RESET_ALL
        for j in range(1, data_map['map_size'] + 1):
            data_ui[i + 4] += '%((' + str(i) + ',' + str(j) + '))5s' + Style.RESET_ALL
        data_ui[i + 4] += '    ' + border_black + border_coloured_right
    ...
    data_ui[data_map['map_size'] + 6] = border_coloured_left + Back.BLACK + (grid_size + 12) * ' ' + Style.RESET_ALL + border_coloured_right
